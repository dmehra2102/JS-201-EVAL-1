...
function studentData(firstName, lastName, age, marksArray, ...rest) {
  // Your solution here.
  return {fullName : `${lastName} ${lastName}`,age,hobbies : rest, getInfo : ()=> `${firstName} ${lastName}'s age is ${age}.`,
      getResult : ()=>{
        let total = marksArray.reduce((acc,item)=> acc+item, 0);

        if((total/marksArray.length) < 50){ return 'Result: FAIL'}
...
function massageArray(inputArray) {
  // Your solution here.
  const output = inputArray.reduce((acc, item)=>{
    let obj = {};
    obj.productId = item.idMeal;
    obj.productTitle = item.strMeal;
    obj.Category = categoriesDirectory[item.Category];
    obj.Area = areas.find((ele)=> ele.id===item.Area).name;
    obj.Ingredients = [];

    let i=1;
    while(item[`strIngredient${i}`]){
      obj.Ingredients.push({ingredient: item[`strIngredient${i}`], measure: item[`strMeasure${i}`]});
      i++;
    }
    acc.push(obj);
    return acc;
